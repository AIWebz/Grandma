...
(function () {
  ...
  function settingsHTML() {
    ...
    return `
      <div class="page-inner">
        ...
        ${GA.Install.available() ? `<div class="section-title" id="set-app">App</div>
        <div class="settings-group">
          <div class="set-row"><div class="l"><b>Get the app</b><small>Install Grandma AI on this ${GA.Install.deviceWord()} — her own icon, full screen, and works offline.</small></div>
            <button class="btn small primary" data-action="install">${U.icon("download")}Install</button></div>
        </div>` : ""}
  ...
  function fillBrain(root) {
    ...
    st.textContent = s.state === "ready" ? "🟢 Awake and ready" + (GA.Brain.engine() === "cpu" ? ` · running on the processor (${GA.Brain.threads() > 1 ? GA.Brain.threads() + " cores" : "1 core"})` : "") : s.state === "loading" ? "🟢 On" : s.state === "error" ? "🟠 Couldn't start — tap Change to try again" : "⚪ Asleep — wakes up when you chat";
  }
